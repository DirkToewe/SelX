...
/** An implementation of the Repeated Step Algorithm as described in:
  ...
  */
object SelectMoMoM3v4 extends Select
{
...
//      if( len < 9*3 ) // <- len < 9 is lowest bound possible
      if( len < 256 ) // <- len < 9 is lowest bound possible
        ...
        var    mid = l+r >>> 1
               mid = mid  min  l + (i  - from)/3 // if `i` far left,  we can move mid to the left  safely
               mid = mid  max  r - (until-i+2)/3 // if `i` far right, we can move mid to the right safely
        select(mid, l,r)
